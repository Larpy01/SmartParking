...
@push('styles')
{{-- Load synchronously in <head> so it's guaranteed ready before our script runs --}}
...
// ─── Wait for DOM then start — library is already loaded via @push('styles') ──
document.addEventListener('DOMContentLoaded', function () {
    if (typeof Html5QrcodeScanner === 'undefined') {
        document.getElementById('qr-reader').innerHTML =
            '<p class="text-red-500 text-sm text-center py-6">QR library failed to load.<br>Please check your internet connection and refresh.</p>';
        return;
    }
    startScanner();
});
