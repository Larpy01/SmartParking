...
@push('scripts')
{{--
    Html5QrcodeScanner is available on window because resources/js/app.js does:
        import { Html5QrcodeScanner } from 'html5-qrcode';
        window.Html5QrcodeScanner = Html5QrcodeScanner;
    Run: npm install html5-qrcode && npm run build
--}}
<script>
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof window.Html5QrcodeScanner === 'undefined') {
        document.getElementById('qr-reader').innerHTML =
            '<p class="text-red-500 text-sm text-center py-6">' +
            'QR scanner unavailable. Please run <code>npm run build</code> and refresh.' +
            '</p>';
        return;
    }
    startScanner();
